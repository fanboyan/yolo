...
def detect(weights=ROOT / 'hangbiao.pt',  # model.pt path(s)
    ...
    # 查询需要检测航标
    select_sql = "SELECT id,vid,camera_para FROM baseline \
               WHERE cid = %s and vid=%s" % (camera_id,last_vid)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        if len(ALL_buoy)>=1:
            ...
            if Video_frames==0:
                ...
                bid=int(ALL_buoy[0][0])
                vid=int(ALL_buoy[0][1])
                ALL_vid.append(vid)
                print("设置PTZ",SetPtz(P=P, T=T, Z=Z))
                #记录开始时间
                # 判断PTZ值，等待并确保摄像头转动成功
                now_PTZ = GetPtz()
            ...
            for i, det in enumerate(pred): # det：所有视频检测出来的信息
                ...
                if begin==1:
                    Video_frames += 1
                    print("当前帧数：", Video_frames)
                    print("成功检测到航标帧数", success_num)
                    print("正在第几个vid：", vid)
                    ...
                    if Video_frames<=end_frames:
                        ...
                        if success_num/end_frames>=0.2:
                            ...
                            print(f_conf)
                            insert_sql = """INSERT INTO visibility_result(cid,set_time,bid,vid,is_visibility,conf)
                                                                    VALUES (%d,'%s',%d,%d,1,%f)
                                                                    """ % (
                                camera_id, start_time, bid,vid,f_conf)
                            ...
                            print("检测完成，识别失败")
                            ALL_conf = []
                            insert_sql = """INSERT INTO visibility_result(cid,set_time,bid,vid,is_visibility)
                                                                                                VALUES (%d,'%s',%d,%d,0)
                                                                                                """ % (
                                camera_id, start_time, bid,vid)
                            Insert_Sql(start_time, host, username, password, db_name, insert_sql)
                            if len(ALL_buoy)>1:
                                ALL_buoy.pop(0)
                                print("进行同级其他航标检测")
                                ...
                                if End_python==False:
                                    select_sql = "SELECT id,vid,camera_para FROM baseline \
                                                   WHERE cid = %s and vid=%s" % (camera_id, vid-1)
                                    ALL_buoy = SELECT_Sql(host, username, password, db_name, select_sql)
                                    if len(ALL_buoy) >=1:
                                        Video_frames = 0
                                        success_num = 0
...
def visibility():
    ...
    if a :
        ...
        # 查询上次时间能见度
        select_sql = "SELECT bid FROM visibility_result WHERE cid =  %s and is_visibility=1 order by id desc limit 1" % (
            camera_id)
        last_bid = SELECT_Sql(host, username, password, db_name, select_sql)
        if len(last_bid) != 0:
            select_sql = "SELECT vid FROM baseline WHERE id = %s" % (last_bid[0][0])
